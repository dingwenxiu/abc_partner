...
@Component({
  selector: 'app-partner-behavior',
  templateUrl: './partner-behavior.component.html',
  styleUrls: ['./partner-behavior.component.scss']
})

export class BackupPartnerBehaviorComponent implements OnInit {
  public tableList = [];
  public dateRange = [];
  public page = {
    index: 1,
  ...
  public async getDataList() {
    ...
    const response = await this.api.Http({ name: 'partnerBehavior', data: { ...this.searchData } });
    const { success, data } = response;
    if (success) {
      this.page.total = data.total;
      this.tableList = data.data;
      this.adminUserList = data.partner_admin_user;
    }
  }
}
